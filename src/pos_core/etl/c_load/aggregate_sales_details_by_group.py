...
- `{GROUP}_subtotal` : numeric subtotal columns for each group (e.g., CAFE_subtotal, DESAYUNO_subtotal)
...
Multiple inputs + show unmapped:
    python aggregate_sales_details_by_group.py -i "detail_2025-09-*.csv" other.csv -o Aggregate.csv --print-unmapped

Batch (folder):
    python aggregate_sales_details_by_group.py --input-dir ./csv --recursive --pattern "*.csv" -o Aggregate.csv
...
# ---------- core ----------
def build_category_pivot(input_csv: str, output_csv: str, include_modifiers: bool | None = None, verbose: bool = False) -> pd.DataFrame:
    """
    ...
    - include_modifiers parameter is kept for API compatibility but not used (ticket CSV doesn't have modifiers)
    """
    ...
    for col_name in subtotal_cols:
        ...
        # Convert underscores back to spaces (since _sanitize_group_name converts spaces to underscores)
    ...
    if extras_misc_groups:
        ...
        for item in extras_misc_groups:
            logger.warning(f"  Column: '{item['column']}' -> Normalized: '{item['normalized']}' -> EXTRAS y MISC")
            if verbose:
    ...
    if verbose:
        ...
        if unmapped_count > 0:
            logger.warning(f"  EXTRAS y MISC: {unmapped_count} rows ({unmapped_count/len(melted)*100:.1f}%)")
    ...
    if sucursal_col:
        ...
        out = agg.set_index("Grupo_Nuevo")[["subtotal_value"]].rename(columns={"subtotal_value": "TOTAL"})
        if verbose:
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Aggregate POS sales details (from ticket-wise CSV with {GROUP}_subtotal columns) by consolidated category (Grupo_Nuevo), pivoted per sucursal."
    )
    p.add_argument(
        "-i", "--input",
        nargs="+",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # _subtotal is 9 characters, not 10!
    if hasattr(str, 'removesuffix'):
        group_names = [_normalize_key(col.removesuffix('_subtotal').replace("_", " ")) for col in subtotal_cols]
    else:
    ...
    pivot = build_category_pivot(tmp_path, args.output, include_modifiers=(not args.no_modifiers), verbose=args.verbose)
