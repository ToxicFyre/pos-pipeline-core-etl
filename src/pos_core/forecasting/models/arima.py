...
class LogARIMAModel(ForecastModel):
    ...
    def train(self, series: pd.Series, **kwargs) -> object:
        ...
        # We try all combinations and select the model with lowest AIC (Akaike Information Criterion)
        # AIC balances model fit with complexity (lower is better)
        # Many combinations will fail (e.g., non-stationary, non-invertible), which is expected
...
def train_log_arima(
    ...
    Returns:
        Tuple of (fitted_model, forecast_series) where forecast is back-transformed to original scale
    """
    model = LogARIMAModel(
