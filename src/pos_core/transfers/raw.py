"""Bronze layer: Raw transfer data extraction.
...
from pos_core.transfers.metadata import should_run_stage
...
def fetch(
    ...
    *,
    ...
    """Ensure raw transfer data exists for the given range.

    Downloads raw transfer Excel files from Wansoft API if needed, depending on mode:
    ...
    if mode == "force":
        logger.info("Mode=force: downloading all transfers for %s to %s", start_date, end_date)
        download_transfers(paths, start_date, end_date, branches)
        return

    # mode="missing": check if extraction is needed
    if should_run_stage(paths.raw_transfers, start_date, end_date, "extract_v1"):
        logger.info("Downloading missing transfers for %s to %s", start_date, end_date)
        download_transfers(paths, start_date, end_date, branches)
    else:
        logger.debug("Raw transfers already exist for %s to %s", start_date, end_date)
...
def load(
    ...
    meta = read_metadata(paths.raw_transfers, start_date, end_date)
    if meta is None or meta.status != "ok":
        raise FileNotFoundError(
            f"Raw transfer data not found for range {start_date} to {end_date}. "
            f"Use transfers.raw.fetch() to download data."
        )
