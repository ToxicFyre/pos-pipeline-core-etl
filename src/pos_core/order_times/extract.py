"""Bronze layer: Download order times reports from Wansoft API.

This module handles downloading raw order times Excel files from the POS system.
The files are saved to the bronze layer (data/a_raw/order_times/).
"""
...
from pos_core.order_times.metadata import StageMetadata, write_metadata

logger = logging.getLogger(__name__)


def download_order_times(
    paths: DataPaths,
    start_date: str,
    end_date: str,
    branches: list[str] | None = None,
) -> None:
    """Download raw order times Excel files from Wansoft API.
    ...
    logger.info("Downloading order times for %s to %s", start_date, end_date)
    ...
    try:
        ...
        for branch_name, segments in branch_segments.items():
            ...
            for segment in segments:
                ...
                try:
                    suggested, blob = export_report(
                        s=session,
                        base_url=base_url,
                        descriptor=ORDER_TIMES_DESCRIPTOR,
                        subsidiary_id=code,
                        start=start_dt,
                        end=end_dt,
                    )
                    ...
                    out_name = build_out_name(
                        "OrderTimes", branch_name, start_dt, end_dt, suggested
                    )
                    out_path = paths.raw_order_times / out_name
        ...
        write_metadata(paths.raw_order_times, start_date, end_date, metadata)

    except Exception as e:
        logger.error("Error downloading order times: %s", e)
        metadata = StageMetadata(
            start_date=start_date,
            end_date=end_date,
            branches=downloaded_branches,
